document.addEventListener('DOMContentLoaded', function() {
    ...
    const fechaHoraInicio = `${year}-${month}-${day}T${hours}:${minutes}`;
    document.getElementById('inicio').value = fechaHoraInicio;
    document.getElementById('inicio').min = fechaHoraInicio;
    
    const tresHorasDespues = new Date(now.getTime() + (3 * 60 * 60 * 1000));
    const termYear = tresHorasDespues.getFullYear();
    const termMonth = (tresHorasDespues.getMonth() + 1).toString().padStart(2, '0');
    const termDay = tresHorasDespues.getDate().toString().padStart(2, '0');
    const termHours = tresHorasDespues.getHours().toString().padStart(2, '0');
    const termMinutes = tresHorasDespues.getMinutes().toString().padStart(2, '0');
    const fechaHoraTermino = `${termYear}-${termMonth}-${termDay}T${termHours}:${termMinutes}`;
    document.getElementById('termino').value = fechaHoraTermino;
    document.getElementById('termino').min = fechaHoraInicio;
      
    regionSelect.addEventListener('change', function() {
        const selectedRegionNumero = parseInt(this.value, 10);
        comunaSelect.innerHTML = '<option value="">Seleccione una comuna</option>';
        comunaSelect.disabled = true;
        
        if (selectedRegionNumero) {
            const selectedRegionData = region_comuna.regiones.find(region => region.numero === selectedRegionNumero);
            
            if (selectedRegionData && selectedRegionData.comunas) {
                selectedRegionData.comunas.forEach(comuna => {
                    const option = document.createElement('option');
    ...
    temaSelect.addEventListener('change', function() {
        if (this.value === 'otro') {
            otroTemaContainer.classList.remove('hidden');
            otroTemaInput.required = true;
        } else {
            otroTemaContainer.classList.add('hidden');
    ...
    agregarFotoBtn.addEventListener('click', function() {
        const fotoGrupos = fotosContainer.querySelectorAll('.foto-grupo');
        
        if (fotoGrupos.length < 5) {
            const nuevoGrupo = document.createElement('div');
            nuevoGrupo.className = 'foto-grupo';
            nuevoGrupo.innerHTML = `
                <input type="file" class="foto-input" name="fotos[]" accept="image/*">
                <button type="button" class="eliminar-foto" style="margin-left: 5px;">X</button>
            `;
            
            fotosContainer.insertBefore(nuevoGrupo, agregarFotoBtn);

            nuevoGrupo.querySelector('.eliminar-foto').addEventListener('click', function() {
                nuevoGrupo.remove();
            ...
            const todosLosInputs = fotosContainer.querySelectorAll('.foto-input');
            todosLosInputs.forEach((input, index) => {
                input.required = index === 0;
            });
        }
        
        if (fotosContainer.querySelectorAll('.foto-grupo').length >= 5) {
            agregarFotoBtn.style.display = 'none';
        }
    });

    const primerInputFoto = fotosContainer.querySelector('.foto-input');
    if (primerInputFoto) {
        primerInputFoto.required = true;
    }
    ...
    formulario.addEventListener('submit', function(e) {
        ...
        const fechaTermino = document.getElementById('termino').value;
        
        if (fechaTermino && new Date(fechaTermino) <= new Date(fechaInicio)) {
            errorMessages.push('La fecha de término debe ser posterior a la fecha de inicio.');
            valido = false;
        }
        
        const sectorInput = document.querySelector('input[name="sector"]');
        if (sectorInput.value.length > 100) {
            errorMessages.push('El sector no puede exceder los 100 caracteres.');
            valido = false;
        }
        
        const nombreInput = document.querySelector('input[name="nombre"]');
        if (nombreInput.value.length > 200) {
            errorMessages.push('El nombre no puede exceder los 200 caracteres.');
            valido = false;
        }
        
        const emailInput = document.querySelector('input[name="email"]');
        if (emailInput.value.length > 100) {
            errorMessages.push('El email no puede exceder los 100 caracteres.');
            valido = false;
        }
        ...
        if (temaSelect.value === 'otro' && otroTemaInput.required) {
            if (otroTemaInput.value.length < 3 || otroTemaInput.value.length > 15) {
                errorMessages.push('El tema personalizado debe tener entre 3 y 15 caracteres.');
                valido = false;
            }
        }
        ...
        let primerInputConArchivo = false;
        
        fotoInputs.forEach((input, index) => {
            if (input.files.length > 0) {
                fotosSeleccionadas++;
                if (index === 0) {
                    primerInputConArchivo = true;
                }
            }
        });
        
        if (fotosSeleccionadas < 1 && fotoInputs.length > 0 && fotoInputs[0].required) {
            errorMessages.push('Debe seleccionar al menos una foto (la primera es obligatoria).');
            valido = false;
        } else if (!primerInputConArchivo && fotoInputs.length > 0 && fotoInputs[0].required) {
            errorMessages.push('La primera foto es obligatoria.');
            valido = false;
        }

        if (fotosSeleccionadas > 5) {
            errorMessages.push('No puede seleccionar más de 5 fotos.');
            valido = false;
        }

        if (!formulario.checkValidity()) {
            errorMessages.push('Hay campos requeridos sin completar o con formato inválido.');
            valido = false;
